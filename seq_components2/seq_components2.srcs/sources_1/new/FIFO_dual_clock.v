...
// Engineer: 
// 
// Create Date: 25.05.2024 15:19:40
// Design Name: 
...
module FIFO_dual_clock(
 input rst,clk1,clk2,wr_en,rd_en,
...
//clk2 for reading 
    ...
    always@(posedge clk1 or posedge rst) begin // always block for updating fifo_counter based on status of FIFO flags and read-write opeerations performed
        if(rst)         // asynchronous rst
            fifo_counter<=0;    
        else if(!buf_full && wr_en) // when writing in FIFO fifo_counter increamented by one unit
            fifo_counter<=fifo_counter+1;
//        else  // this part of code can create conflict for the value of fifo_counter
//            fifo_counter<=fifo_counter; // other than these three conditions keep fifo_counter as it is.
                
    end
    
     always@(posedge clk2 or posedge rst ) begin // always block for updating fifo_counter based on status of FIFO flags and read-write opeerations performed
        if(rst)
        fifo_counter<=0;
        else 
        if(!buf_empty && rd_en) // when reading from FIFO fifo_counter dectremented by one unit
            fifo_counter<=fifo_counter-1;
        else // this part of code can create conflict for the value of fifo_counter 
            fifo_counter<=fifo_counter; // other than these three conditions keep fifo_counter as it is.
    end
   ...
   always@(posedge clk1 or posedge rst) begin //always block for read and write pointer update whenever read or write operation performed
           if(rst) 
           wr_ptr<=0;
   ...
   always@(posedge clk2 or posedge rst) begin //always block for read and write pointer update whenever read or write operation performed
           if(rst) begin
           rd_ptr<=0;
           end
           else begin
